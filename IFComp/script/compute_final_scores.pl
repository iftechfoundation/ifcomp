...
my $ifcomp_sql =
    "select entry.id as entry_id, round(avg(score), 2) as average_score, round(std(score), 2) as standard_deviation, count(score) as votes_cast, sum(score = 1) as total_1, sum(score = 2) as total_2, sum(score = 3) as total_3, sum(score = 4) as total_4, sum(score = 5) as total_5, sum(score = 6) as total_6, sum(score = 7) as total_7, sum(score = 8) as total_8, sum(score = 9) as total_9, sum(score = 10) as total_10 from ( select vote.* from vote left join entry on vote.user = entry.author and entry.comp = ? where user in ( select user from vote, entry where entry.id = vote.entry and entry.comp = ? group by user having count(score) >= 5 ) group by vote.id ) judge_vote, entry where entry.id = judge_vote.entry and entry.comp = ? and entry.is_disqualified = 0 group by entry.id order by avg(score) desc;";
my $mc_sql =
    "select entry.id as entry_id, round(avg(score), 2) as average_score from ( select distinct vote.* from vote, entry where vote.user = entry.author and entry.is_disqualified = 0 and entry.comp = ? ) judge_vote, entry where entry.id = judge_vote.entry and entry.comp = ? group by entry.id order by average_score desc";
...
$mc_sth->execute( $current_comp->id, $current_comp->id, );
