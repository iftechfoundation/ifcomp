...
my $mc_sql = 'select entry.id as entry_id, round(avg(score), 2) as average_score from ( select vote.* from vote left join entry on vote.user = entry.author and entry.comp = ? where entry.is_disqualified = 0 ) judge_vote, entry where entry.id = judge_vote.entry group by entry.id order by average_score desc';
